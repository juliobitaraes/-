...
export function extendUsuarios(app) {
    ...
    app.baixarAtividadeAvulsaPDF = async function(id, incluirGabarito = false) {
        ...
        try {
            ...
            const questionsHtml = (atividade.questions || []).map((question, index) => { const options = Array.isArray(question.options) ? question.options : []; const correctIndex = Number.isInteger(question.correct) ? question.correct : 0; return `<div style="margin-bottom:18px;border-bottom:1px solid #e5e7eb;padding-bottom:12px"><strong>${index + 1}. ${safe(question.text || '')}</strong><ol type="A" style="margin-top:8px">${options.map((option, optionIndex) => `<li style="${incluirGabarito && optionIndex === correctIndex ? 'font-weight:700;color:#047857' : ''}">${safe(option)}${incluirGabarito && optionIndex === correctIndex ? ' (correta)' : ''}</li>`).join('')}</ol></div>`; }).join('');
            const container = document.createElement('div'); container.style.cssText = 'background:#fff;color:#111;padding:24px;font-family:Arial,sans-serif;width:720px'; container.innerHTML = `<h1 style="margin:0 0 6px">${safe(atividade.titulo || 'Atividade Avulsa')}</h1><p style="margin:0 0 20px;color:#64748b">${incluirGabarito ? 'Gabarito' : 'Atividade Avulsa'} | ${(atividade.questions || []).length} questão(ões)</p>${questionsHtml}`; document.body.appendChild(container);
            await html2pdf().set({ margin: 0.4, filename: `${incluirGabarito ? 'Gabarito' : 'Atividade_Avulsa'}_${String(atividade.titulo || 'atividade').replace(/[^a-z0-9]+/gi, '_')}.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2 }, jsPDF: { unit: 'in', format: 'a4', orientation: 'portrait' } }).from(container).save(); container.remove();
        } catch (error) { console.error('Erro ao gerar PDF da atividade avulsa:', error); alert(`Erro ao gerar PDF: ${error.message || error}`); }
    };
